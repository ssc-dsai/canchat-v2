...
import { showAdminSettings, waitToSettle, toggleSwitch, sendChatMessage, showStartPage, showSidebar } from '../../src/utils/navigation';
...
test('Test - Image Generation Availability', async ({ page }, testInfo) => {
  ...
  const response = await sendChatMessage(page, 'Make a picture of the Easter Bunny.', 'Generating an image');
  await waitToSettle(page, 1000);

  const screenshot = await page.screenshot({ path: './playwright-report/screenshots/image_gen.png' });
