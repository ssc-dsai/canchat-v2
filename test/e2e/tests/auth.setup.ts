...
async function populateAccounts({ page }: { page: Page }) {
	...
	await page
		.locator("//div[@contenteditable='true'][@data-placeholder='Enter English description']")
		.fill('chatgpt');
	await page
		.locator("//div[@contenteditable='true'][@data-placeholder='Enter French description']")
		.fill('chatgpt');
...
setup('authenticate', async ({ page }) => {
	...
	for (const user of credentials.users) {
		...
		if (await okButton.isVisible({ timeout: 1000 })) await okButton.click();
