...
export async function showSidebar(page: Page, visibility: 'show' | 'hide' | 'toggle' = 'show') {
    ...
    if ((visibility === 'show' && visibleButton === toggleButton) ||
...
export async function setLanguage(page: Page, language: 'en' | 'fr' | 'toggle' = 'en') {
    ...
    const engButton = await page.locator("//button[contains(@class,'group')][./div[contains(normalize-space(text()),'FR')]]")
    const frButton = await page.locator("//button[contains(@class,'group')][./div[contains(normalize-space(text()),'EN')]]")
    ...
    if ((language === 'en' && languageButton !== engButton) ||
...
export async function showAdminSettings(page: Page, menu: string = 'Settings', section: string = 'General') {
    await page.goto('/');
...
export async function sendChatMessage(page: Page, message: string, idleMessage: string = ''): Promise<string> {
    ...
    await page.waitForFunction(
        ({ selector, idleMessage }) => {
            const el = document.querySelector(selector); console.log('el', el?.textContent);
            if (!el) return false;
        ...
        { selector, idleMessage }, { timeout: 180000 }
    );
