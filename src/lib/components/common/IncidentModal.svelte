<script lang="ts">
	...
	const submitReport = async () => {
		if (loading) return; // Guard against multiple submissions
		...
		try {
			...
			const res = await createIncident(localStorage.token, {
				email: $user?.email || '',
				description,
				stepsToReproduce,
				files
			...
			success = true;
			setTimeout(() => {
				loading = false; // Set this before closing
				closeModal();
			}, 2000);
		} catch (error) {
			error =
			...
			loading = false; // Make sure to reset on error
		}
	};
